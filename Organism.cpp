...
void Organism::updateInputs() {
    ...
    for(auto & [neuronID, activation] : activations) {
        switch(neuronID) {
            case HUNGER: {
                activation = static_cast<float>((-hunger) + 100);
            }
            case FOOD_LEFT: {
                activation = 1.00f;
            }
...
void Organism::tryEat(const float activation) {
    ...
    std::vector<uint64_t> collidingIDs = simState.quadTreePtr->query(
        QuadTree::QuadTreeObject(id, SDL_FRect{boundingBox.x, boundingBox.y, boundingBox.w, boundingBox.h}));
